...
export default function PropertyDetailPage() {
  ...
  if (isError || !property) return <p className="text-destructive">Property not found.</p>
  ...
  function handleDelete() {
    ...
    deleteProperty.mutate(property!.id, {
      onSuccess: () => navigate('/properties/my'),
    })
  }
    ...
    <div className="max-w-2xl">
      <div className="flex items-start justify-between mb-6 gap-4">
        <div>
          ...
          <p className="text-muted-foreground">{property.address}, {property.city} {property.postalCode}</p>
        </div>
      ...
      <Card className="mb-4">
        ...
        <CardContent className="grid grid-cols-2 gap-3 text-sm">
          ...
          <div>
            ...
            <p className="font-medium">${property.pricePerMonth.toLocaleString()}</p>
          </div>
          {property.bedrooms && (
            <div>
          ...
          <CardContent className="space-y-3">
            <div className="flex items-center gap-3">
              <Select
                ...
                onValueChange={(v) => updateStatus.mutate({ id: property.id, status: v })}
              >
